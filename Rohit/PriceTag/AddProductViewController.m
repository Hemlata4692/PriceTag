...
//    self.weight.frame=CGRectMake(12, 260, 162, 40);
//    self.weightImgView.frame=CGRectMake(12, 260, 162, 40);
//    self.weightPNG.frame=CGRectMake(18, 268, 21, 21);
//    
//    self.measure.frame=CGRectMake(188, 260, 121, 40);
//    
//    self.submitOutlet.frame=CGRectMake(12, 358, 297, 40);
    
    
    NSArray *fields = @[_productName,_barCode,_weight,_measure];
...
//            self.keyboardControls.fields=@[_productName,_barCode,_other,_weight,_measure];
//        }
//        else
//        {
...
//            self.submitOutlet.frame=CGRectMake(12, 358, 297, 40);
            ...
            //set bskeyboardcontrols fields
            self.keyboardControls.fields=@[_productName,_barCode,_weight,_measure];
//        }
...
-(void)addProduct
{
    ...
    if ([internet start])
    {
        ...
        NSString *strEncoded;
        UIImage* add_photoCheck = [UIImage imageNamed:@"add_photo.png"];
        NSData *add_photoCheckData = UIImagePNGRepresentation(add_photoCheck);
        NSData *profileImageData = UIImagePNGRepresentation(_addPhoto.image);
        if ([profileImageData isEqualToData:add_photoCheckData ])
        {
            strEncoded = @"";
        }
        else
        ...
        //    else
        //    {
        //        requestDict = @{@"product_image":strEncoded,@"user_id":myDelegate.userId,@"product_name":self.productName.text,@"product_barcode":self.barCode.text,@"store_id":[NSNumber numberWithInt:storeID],@"weight":[NSNumber numberWithDouble:[self.weight.text doubleValue]],@"unit":self.measure.text};
        requestDict = @{@"product_image":strEncoded,@"user_id":myDelegate.userId,@"product_name":self.productName.text,@"product_barcode":self.barCode.text,@"weight":[NSNumber numberWithDouble:[self.weight.text doubleValue]],@"unit":self.measure.text};
        //    }
        ...
        if ([[tempDict valueForKey:@"isSuccess"] intValue] == 1)
        {
            ...
            objProductDetail.productId=[tempDict objectForKey:@"productId"];
//            [self.navigationController popViewControllerAnimated:YES];
            [self.navigationController pushViewController:objProductDetail animated:YES];
            
//            [NSThread detachNewThreadSelector:@selector(addPriceMonitor:) toTarget:self withObject:tempDict];
...
//-(void)addPriceMonitor:(NSMutableDictionary*)tempDict{
//   
//    NSDictionary *requestDict;
//    NSLog(@"%@,%@",myDelegate.userId,[tempDict objectForKey:@"productId"]);
////    requestDict = @{@"user_Id":[NSNumber numberWithInt:4],@"product_Id":@"78"};
//    requestDict = @{@"user_Id":myDelegate.userId,@"product_Id":[tempDict objectForKey:@"productId"]};
//    //    }
